...
GLFWbool _glfwInitEGL(void)
{
    ...
    _glfw.egl.prefix = (strncmp(sonames[i], "include", 3) == 0);
                               ...
                               const _GLFWfbconfig* fbconfig)
{
    ...
    // Load the appropriate client library
    {
        ...
        for (i = 0;  sonames[i];  i++)
        {
            // HACK: Match presence of include prefix to increase chance of finding
            //       a matching pair in the jungle that is Win32 EGL/GLES
            if (_glfw.egl.prefix != (strncmp(sonames[i], "include", 3) == 0))
                continue;
